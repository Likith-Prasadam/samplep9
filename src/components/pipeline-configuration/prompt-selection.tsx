...
export const PromptSelectionField = ({
  ...
  // Tracks whether an async parent-resolution search is in flight so the
  // auto-select effect doesn't override the saved value while we're searching.
  const parentSearchInProgressRef = useRef(false);
  // Tracks the version hash we last kicked off a parent search for, so we
  // don't start a redundant search on every render.
  const resolvedForValueRef = useRef<string>('');
  ...
  // We support both:
  // - simple types like "system", "user", "events_list"
  // - fully-qualified types like "event_detection/__system"
  const matchingPrompts = useMemo(() => {
    const matches = accessiblePrompts.filter((p) => {
      const pType = p.promptType as string | undefined;
      if (!pType) return false;

      const isMatch = requiredTypes.some((t) => {
        if (!t) return false;
        return (
          pType === t ||
          pType.startsWith(t) ||
          // Handle namespaced types like "event_detection/__system"
          pType.endsWith(t) ||
          pType.includes(`/${t}`) ||
          pType.includes(`__${t}`)
        );
    ...
    console.log(`[PromptSelection] Filtering for label "${label}":`, {
      requiredTypes,
      totalPrompts: accessiblePrompts.length,
      matchingPrompts: matches.length,
      allPromptTypes: accessiblePrompts.map((p) => p.promptType),
      matchedTypes: matches.map((p) => ({
        name: p.promptName,
        type: p.promptType,
      })),
    });
    ...
    return matches;
  }, [accessiblePrompts, requiredTypes, label]);
  ...
  // Keep selected parent in sync with saved parent hash from config (GET/refetch).
  useEffect(() => {
    if (!selectedParentHash) return;
    if (selectedParentHash === selectedParent) return;

    setSelectedParent(selectedParentHash);
  }, [selectedParentHash, selectedParent]);

  // Async helper: search all parent templates' version lists to find which one
  // owns the saved version hash. Used when editing a config whose parent hash
  // wasn't persisted to the backend.
  const resolveParentForValue = useCallback(
    async (versionHash: string, parents: { parentHash: string }[]) => {
      if (parentSearchInProgressRef.current) return;
      parentSearchInProgressRef.current = true;
      try {
        const results = await Promise.all(
          parents.map((p) =>
            apolloClient
              ...
              .catch(() => ({ parentHash: p.parentHash, versions: [] }))
          )
        );
        const match = results.find((r) =>
          r.versions.some(
            (v) => (v.promptHash as string | undefined) === versionHash
          )
        );
        if (match) {
          setSelectedParent(match.parentHash);
        } else if (parents.length > 0) {
          // Fallback: best-effort pick first parent
          setSelectedParent(parents[0].parentHash);
        }
      } finally {
        parentSearchInProgressRef.current = false;
      }
    },
    [apolloClient]
  );
  ...
  // Priority order:
  //  1. selectedParentHash prop (edit form with stored parent) — handled by sync effect above
  //  2. value is a version hash with no stored parent → search all parents to find owner
  //  3. No value at all (add form) → pick parentOptions[0] so auto-fill can run
  useEffect(() => {
    if (parentOptions.length === 0) return;
    if (selectedParent) return;

    // Case 1: parent hash provided explicitly (already handled by sync effect, but
    // guard here too so we don't race it with parentOptions[0])
    if (selectedParentHash) return;

    // Case 2: saved version hash exists but parent unknown → find it asynchronously
    if (value && resolvedForValueRef.current !== value) {
      resolvedForValueRef.current = value;
      resolveParentForValue(value, parentOptions);
      return;
    }

    // Case 3: no value yet (add form) → pick first parent so versions load & auto-fill fires
    if (!value) {
  ...
  // Auto-select the latest version when versions load and no valid value is set.
  // Guarded so it never fires while an async parent search is resolving — the
  // parent will switch to the correct template and this effect will re-run.
  useEffect(() => {
    if (!selectedParent) return;
    ...
    if (!valueValid && !isRecentManualSelection) {
      ...
      onChange(latest.promptHash as string, {
        promptName: versionDisplay,
        parentPromptHash: selectedParent,
      });
    }
