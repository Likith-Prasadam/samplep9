...
const markdownComponents: MarkdownComponents = {
  ...
  table: ({ children, ...props }) => (
    <div className="my-4 rounded-xl border border-border/70 border-l-4 border-l-primary/60 bg-muted/60 shadow-sm overflow-x-auto spectra-scrollbar">
  ...
  th: ({ children, ...props }) => (
    <th
      className="px-3 py-1.5 font-semibold border border-border/40 whitespace-nowrap text-foreground"
      {...props}
    >
      {children}
    </th>
  ...
  td: ({ children, ...props }) => (
    <td
      className="px-3 py-1.5 border border-border/40 align-top whitespace-nowrap text-foreground"
      {...props}
    >
      {children}
    </td>
...
// Normalize "flattened" markdown tables
...
const Message: React.FC<{
  ...
  const splitContent = (text: string) => {
    ...
    while ((lastCloseMatch = closeRegex.exec(decodedText)) !== null) {
    ...
    const hasOpenTag = openRegex.test(decodedText);
    ...
    if (hasOpenTag) {
      ...
      return { toolsPart: decodedText, textPart: '', hasOpenTag: true };
    }

    if (lastCloseIndex !== -1) {
      // We have closed tools and no open ones after.
      const toolsPart = decodedText.slice(0, lastCloseIndex);
      const textPart = decodedText.slice(lastCloseIndex).trim();
      return { toolsPart, textPart, hasOpenTag: false };
    }

    // No tools found.
    return { toolsPart: '', textPart: decodedText, hasOpenTag: false };
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {message.role === 'assistant'
          ...
          : message.content.map((t, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="rounded-2xl rounded-tr-sm px-4 py-3 shadow-sm max-w-full bg-blue-100 text-slate-900 dark:bg-blue-500 dark:text-white"
                >
        ...
        {/* Show spinner if loading and this is the last assistant message */}
        {isAssistant && isLastMessage && isStreaming && !fullText.trim() && (
          <div className="rounded-2xl rounded-tl-sm px-4 py-3 shadow-sm bg-card text-foreground border border-border/60">
            <div className="text-xs font-semibold mb-1.5 text-primary">
              Spectra
            </div>
        ...
        {(message.timestamp || fullText.trim()) && (
          ...
          >
            {message.timestamp && <span>{convertTime(message.timestamp)}</span>}
            {fullText.trim() && (
              <button
                type="button"
                onClick={handleCopy}
...
const ChatMessages = forwardRef<ChatMessagesRef, ChatMessagesProps>(
  ({ conversation, isLoading, isStreaming }, ref) => {
    ...
    // Respect user scrolling even during streaming.
    useEffect(() => {
      if (!isUserScrolling && conversation.length > 0) {
    ...
    return (
      <div ref={scrollAreaWrapperRef} className="flex-1 h-full">
        <ScrollArea className="h-full">
          <div
            className="h-full space-y-6 pr-6 pl-6 py-4"
            style={{ minHeight: '100%' }}
          >
            {conversation.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center h-full text-center space-y-4">
                ...
                <h2 className="text-lg font-semibold">Welcome to Spectra</h2>
