...
type UseTableUrlStateParams = {
  ...
  navigate: NavigateFn;
  /**
   * Debounce (ms) for global filter URL updates.
   * Helps keep input focus smooth by avoiding URL changes on every keystroke.
   */
...
export function useTableUrlState(
  ...
  // Debounce URL updates for global search to avoid input focus loss.
  const globalFilterDebounceTimer = useRef<ReturnType<
    typeof setTimeout
  > | null>(null);

  useEffect(() => {
    return () => {
      if (globalFilterDebounceTimer.current) {
        clearTimeout(globalFilterDebounceTimer.current);
      }
    };
  }, []);
    ...
    globalFilterEnabled
      ? (updater) => {
          ...
          if (globalFilterDebounceMs > 0) {
            ...
            globalFilterDebounceTimer.current = setTimeout(
              () => runNavigate(),
